...
bool get isLoading => _isLoading;
 ...
 Future<bool> getAllProducts(BuildContext context,{required bool isLoadMore}) async{
   _isLoading=true;
   bool isDone=false;
   _productListRespponseModel=null;
   if(isLoadMore){
   ...
   try{
     ...
     if(apiResponse.responseValue != null){
      if(apiResponse.responseValue!.statusCode == 200){
        ...
        if(context.mounted){
          ScaffoldMessenger.of(context).removeCurrentSnackBar();
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(duration: Duration(seconds: 3),content: Text('Successfull')));
        }
      }else{
        _isLoading=false;
        isDone=false;
